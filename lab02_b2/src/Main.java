...
public class Main
{
    public static void main(String[] args) {
        ...
        System.out.print("ID (7 char): ");
        String studentId = scanner.nextLine();
        System.out.print("Giới tính (Female / Male): ");
        ...
        if (!studentIdNumber.matches(".*[0-9].*")) {
            System.out.println("ERR: Student Invalid StudentID");
            return;
        }

        if (studentId.length() != 7 || !studentId.matches(".*[0-9].*")) {
            System.out.println("ERR: Student Invalid CitizenID");
            return;
        }

        if (!studentSex.equalsIgnoreCase("Female") && !studentSex.equalsIgnoreCase("Male")) {
            System.out.println("ERR: Student Giới tính phải là Female hoặc Male");
            return;
        }
        ...
        if (numberOfSubjects < 1 || numberOfSubjects > 5) {
            System.out.println("ERR: Student Invalid number of subjects");
            return;
        }
        ...
        for (int i = 0; i < numberOfSubjects; i++) {
            ...
            if (subjectMark < 0 || subjectMark > 10) {
                System.out.println("ERR: Student Invalid mark of subject");
                return;
            }
        ...
        if (employeeId.length() != 7 || !employeeId.matches(".*[0-9].*")) {
            System.out.println("ERR: Employee Invalid CitizenID");
            return;
        }

        // Kiểm tra ràng buộc cho giới tính
        if (!employeeSex.equalsIgnoreCase("Female") && !employeeSex.equalsIgnoreCase("Male")) {
            System.out.println("ERR: Employee Giới tính phải là Female hoặc Male");
            return;
        }

        // Kiểm tra ràng buộc cho lương cơ bản
        if (baseSalary < 1000) {
            System.out.println("ERR: Employee Lương cơ bản tối thiểu là 1000");
            return;
        }
